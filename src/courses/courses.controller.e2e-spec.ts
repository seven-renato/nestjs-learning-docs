...
describe('CoursesController e2e tests', () => {
  ...
  beforeAll(async () => { // Subir um modulo da aplicação apenas uma vez
    const module: TestingModule = await Test.createTestingModule({
      imports: [
        CoursesModule, TypeOrmModule.forRootAsync({
          useFactory: async () => {
    ...
    await app.init() // Iniciou a aplicação inteira
  ...
  beforeEach( async () => {
    const dataSource = await new DataSource(dataSourceTest).initialize()// Começar a conexão
